...
mod_allometry_server_morphometric <- function(id, raw_data_r) {
  moduleServer(id, function(input, output, session) {
    ...
    allom_modified <- function(data, type) {
      original_col_names <- colnames(data)
      otu_col_name <- original_col_names[1]
      body_size_col_name <- original_col_names[2]
      trait_col_names <- original_col_names[3:ncol(data)]
      
      data_ordered <- data[order(data[,1]), ]
      ...
      if (type %in% c("species", "population1", "population2")) {
        for (i in unique(data_ordered[,1])) {
          if (sum(data_ordered[,1] == i) < 2) {
            stop(paste("Error: For type '", type, "', OTU '", i, "' has less than 2 individuals. Each OTU must have more than one individual for allometric adjustment."))
          }
        }
      }
      
      # Ensure body-size column (2nd column) and trait columns (3rd onwards) are positive
      if (any(data_ordered[[body_size_col_name]] <= 0)) {
        stop("Error: Body-size (2nd column) must contain only positive values for log transformation.")
      }
      if (any(data_ordered[, trait_col_names] <= 0)) {
        stop("Error: Trait columns (3rd onwards) must contain only positive values for log transformation.")
      }
      
      # Create a copy of the data to store adjusted values, maintaining original structure
      ...
      # Log-transform all traits for internal calculation (used by species/population1 logic)
      ...
      if (type == "species") {
        ...
        # Each group has its own slope and its own mean body size for adjustment
        for (current_otu in unique(data_ordered[,1])) {
          species_subsets_indices <- which(data_ordered[,1] == current_otu)
          species_subsets_log <- df_log_internal[species_subsets_indices, ]
          
          log_y_subset <- species_subsets_log[[body_size_col_name]]
          z_mean <- mean(data_ordered[[body_size_col_name]][species_subsets_indices]) # Mean of original body size for this group
          log_z_mean <- log10(z_mean)
          
          for (trait_name in trait_col_names) {
            log_x_subset <- species_subsets_log[[trait_name]]
            
            # Check for sufficient data points and variance for regression within the group
            if (length(log_y_subset) > 1 && sd(log_y_subset) > 0 && length(unique(log_x_subset)) > 1) {
              ...
              beta <- 0 # If no slope can be calculated (e.g., constant values, insufficient data), assume 0 slope (no allometry)
        ...
        # All populations share a common slope, but adjustment is relative to each population's mean body size
        
        # Calculate common slopes for each trait across all data (pooled)
        common_betas <- list()
        for (trait_name in trait_col_names) {
          formula_full <- paste0("`", trait_name, "` ~ `", body_size_col_name, "`")
          if (nrow(df_log_internal) > 1 && sd(df_log_internal[[body_size_col_name]]) > 0 && length(unique(df_log_internal[[trait_name]])) > 1) {
            lm_model_full <- lm(as.formula(formula_full), data = df_log_internal)
            common_betas[[trait_name]] <- as.numeric(lm_model_full$coefficients[body_size_col_name])
          } else {
            common_betas[[trait_name]] <- 0 # Default to 0 if no common slope can be calculated
        ...
        for (current_otu in unique(data_ordered[,1])) {
          ...
          log_y_subset <- species_subsets_log[[body_size_col_name]]
          z_mean <- mean(data_ordered[[body_size_col_name]][species_subsets_indices]) # Mean of original body size for this population
          log_z_mean <- log10(z_mean)
          
          for (trait_name in trait_col_names) {
            log_x_subset <- species_subsets_log[[trait_name]]
            beta <- common_betas[[trait_name]] # Use the common beta
        ...
        for (trait_name in trait_col_names) {
          ...
          # Apply adjustment and update directly in adjusted_df_output
    ...
    observe({
      req(raw_data_r(), input$correction_type)
      ...
      adjusted_df <- tryCatch({
        allom_modified(data = df_raw, type = input$correction_type)
      }, error = function(e) {
