...
mod_inferential_ui_combined <- function(id) {
  ns <- NS(id)
  tagList(
    h3("Inferential Statistics (Morphometric Data Only)"),
    hr(),
    ...
    tags$div(
      ...
      p(style = "margin: 0;",
        strong("Notice:"),
        "This module performs inferential statistics exclusively on morphometric traits and uses the size-corrected data from the Allometric Correction submodule. The allometric correction must be run first with morphometric traits selected. All analyses in this module are performed on the adjusted morphometric data output from the Allometric Correction submodule."
      )
    ),
...
mod_inferential_server_combined <- function(id, data_r) {
  moduleServer(id, function(input, output, session) {
    ...
    output$download_pca_summary <- downloadHandler(
      filename = function() {
        paste0("pca_summary_combined_", Sys.Date(), ".csv")
      },
    ...
    observeEvent(input$main_tabs, {
      if (input$main_tabs == "Multivariate (PERMANOVA)") {
        ...
        output$download_all_permanova <- downloadHandler(
          filename = function() paste0("permanova_results_combined_", Sys.Date(), ".zip"),
          content = function(file) {
            tmpdir <- tempdir()
            files  <- c()
            perm_main <- permanova_main_results_r()
            if (!is.null(perm_main)) {
              f <- file.path(tmpdir, paste0("permanova_main_combined_", Sys.Date(), ".txt"))
              sink(f); print(perm_main); sink()
              files <- c(files, f)
            }
            perm_pair <- permanova_pairwise_results_r()
            if (!is.null(perm_pair)) {
              f <- file.path(tmpdir, paste0("permanova_pairwise_combined_", Sys.Date(), ".csv"))
              write.csv(perm_pair, f, row.names = FALSE)
              files <- c(files, f)
            }
            bd <- betadisper_results_r()
            if (!is.null(bd)) {
              f <- file.path(tmpdir, paste0("betadisper_combined_", Sys.Date(), ".txt"))
              sink(f)
              cat("=== Multivariate Homogeneity of Group Dispersions (betadisper) ===\n\n")
              ...
              files <- c(files, f)
              if (!is.null(bd$tukey)) {
                f <- file.path(tmpdir, paste0("betadisper_pairwise_combined_", Sys.Date(), ".csv"))
            ...
            if (!is.null(df_cent)) {
              f <- file.path(tmpdir, paste0("centroid_distances_combined_", Sys.Date(), ".csv"))
              write.csv(df_cent, f, row.names = FALSE)
              files <- c(files, f)
            }
            df_cons <- consolidated_pairwise_r()
            if (!is.null(df_cons)) {
              f <- file.path(tmpdir, paste0("consolidated_pairwise_combined_", Sys.Date(), ".csv"))
