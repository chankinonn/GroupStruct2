    ...
    tabsetPanel(id = ns("visual_tab"),
                ...
                tabPanel("MFA: Individuals",
                         fluidRow(
                           ...
                           column(3,
                                  br(),
                                  numericInput(ns("mfa_individuals_plot_height"), "Plot Height (px)", value = 600, min = 300, step = 50, width = '100%'), # Added width = '100%'
                                  numericInput(ns("mfa_individuals_plot_width"), "Plot Width (px)", value = 700, min = 200, step = 50, width = '100%'), # ADDED PLOT WIDTH
                                  hr(),
                                  h5("MFA Individuals Plot Settings"),
                                  numericInput(ns("mfa_point_size"), "Point Size:", value = 6, min = 1, max = 10, width = '100%'),
                                  selectInput(ns("mfa_point_shape"), "Point Shape:",
                                              choices = c("Circle" = 19, "Square" = 15, "Triangle" = 17, "Diamond" = 18),
                                              selected = 21),
                                  checkboxInput(ns("mfa_convex_hull"), "Show Convex Hulls", value = FALSE),
                                  checkboxInput(ns("mfa_ellipse"), "Show 95% Confidence Ellipses", value = TRUE),
                                  checkboxInput(ns("mfa_centroids"), "Show Group Centroids", value = FALSE),
                                  sliderInput(ns("mfa_ellipse_alpha"), "Ellipse Transparency (Alpha):", min = 0, max = 1, value = 0.4, step = 0.05, width = '100%'),
