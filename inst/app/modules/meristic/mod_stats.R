    ...
    tabsetPanel(id = ns("main_tabs"), 
                ...
                tabPanel("Multivariate (PERMANOVA)",
                         ...
                         p("The tabs below present: (1) the overall PERMANOVA result, (2) pairwise PERMANOVA comparisons between groups,",
                           "(3) dispersion analysis (betadisper), (4) pairwise dispersion comparisons (Tukey HSD),",
                           "(5) pairwise centroid distances, (6) a consolidated pairwise summary, and (7) an interpretation guide.",
                           "You can choose to run the analysis on the original trait data or on PCA scores."),
                         ...
                         br(), br(),
                         tabsetPanel(
                           tabPanel("Main PERMANOVA", br(), verbatimTextOutput(ns("permanova_main_results"))),
                           tabPanel("Pairwise PERMANOVA", br(),
                           ...
                           tabPanel("BetaDisper", br(),
                                    p("Tests the homogeneity of multivariate dispersions using ", code("vegan::betadisper()"), "."),
                                    verbatimTextOutput(ns("betadisper_results"))),
                           tabPanel("Pairwise Dispersion", br(),
                           ...
                           tabPanel("Centroid Distances", br(),
                                    p("Euclidean distances between group centroids in PCoA space."),
                                    DTOutput(ns("centroid_dist_results"))),
                           tabPanel("Consolidated Summary", br(),
    ...
    observeEvent(input$main_tabs, {
      if (input$main_tabs == "Multivariate (PERMANOVA)") {
        ...
        observeEvent(input$run_permanova, {
          ...
          # Handle NAs by removing incomplete rows
          ...
          if (!is.null(input$use_pca) && input$use_pca) {
            ...
            if (any(zero_var_cols)) {
              ...
              if (ncol(permanova_data_input) < 2) {
            ...
            permanova_data_input <- pca_result$x[, apply(pca_result$x, 2, var) > 1e-9, drop = FALSE]
        ...
        output$betadisper_results <- renderPrint({
          ...
          cat("(Pairwise comparisons shown in Pairwise Dispersion tab)\n\n")
        ...
        output$centroid_dist_results <- renderDT({
          df <- centroid_dist_r()
          if (is.null(df)) return(datatable(data.frame(Message = "Centroid distances not available."),
                                            options = list(dom = 't', paging = FALSE, searching = FALSE)))
